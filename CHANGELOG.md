...
### Added
- **Memory fixes the words it keeps mishearing.** Correct the same mishearing twice ("super base" → Supabase) and Memory swaps it in by itself from then on, before polish, with every local engine (including Parakeet, X-ASR and SenseVoice, which take no word hints) and the batch cloud providers. The "Learned" toast tells you when a swap switches on. A swap edits every future transcript, so Memory never learns one when the misheard text could be something you really said: a real word ("their" → "There"), a number ("10" → "10am"), or a phrase with everyday words ("a team" → Adam). Words you type next to a fix ("Supabase today") aren't learned as part of it. Mishearings you type into Memory yourself apply straight away. Changing a swapped word back makes Memory forget that swap; Undo on the toast or deleting the word removes it too.
