...
- **Default STT switched to NVIDIA Parakeet TDT 0.6b for an instant feel.** Measured local Whisper STT latency (M5 Pro, warm): base ~114 ms, turbo ~865 ms, large-v3 ~1155 ms for a 5 s clip — an 8–10x model-dependent swing, and a fresh/fallback install could land on the slow turbo/large tiers. Parakeet TDT is a fast on-device transducer that's both quicker and more accurate than the comparable Whisper tiers, so it's now the default local provider (`localTranscriptionProvider` → `nvidia`, `parakeetModel` → `parakeet-tdt-0.6b-v3`). Whisper remains available in Settings. Profiling harness added: `eval/bench-stt.sh`.
