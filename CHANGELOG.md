...
- **Settings export includes your Memory when encryption is on.** Export used to skip the encrypted Memory and style examples, and import wrote them back as plain files next to the encrypted ones. Both now go through encryption, and imported words show up in Memory right away (before, the next Memory change could quietly drop them).
- **Plugins ask before they run anything.** Before a plugin's MCP server starts, a macOS dialog shows the exact command, and it runs only if you click Allow. That's once per command per session, and a Don't Allow sticks until you restart. Before, whatever command was in the plugins file just ran, whether it got there from the Plugins screen, a settings import, or a compromised window of the app. The built-in Todoist, TickTick, Notion, Calendar and Slack entries ask too: they start with `npx`, which runs whatever is published under that name. Setting up a plugin can now only set that plugin's own API-key variable, not things like `PATH`.
...
- **A lock that waited for a meeting now always happens.** If WhisperWoof should have locked during a meeting (your Mac slept, say), it locks a minute after the meeting ends. When stopping or starting the meeting failed, it used to stay unlocked until you locked it yourself.
...
### Fixed
- **The install script could launch a stale build.** A leftover `dist/` copy with the same bundle id (1.15.6) was launched instead of the new `/Applications` build. The script now unregisters and removes the build's `dist/` copy after installing.
...
### Added
- **Mando is animated now.** The dictation indicator's static head is replaced by the full Mando character from the v6 animation pack: a head-tilt while it waits for your voice, a nod along while you speak, a chin-scratch think while transcribing and polishing, and a single hop the moment your text lands (auto-hide waits for the hop to finish). The Cmd+K agent bar shows the same thinking Mando while it transcribes or reasons. Animations are CSS `steps()` over four small single-row WebP spritesheets (~320KB total, 2x for Retina, 12.5fps) in `src/assets/mando`, built by `scripts/build-mando-sprites.js` from the 78MB source pack, which stays out of git. State → animation mapping (and the hop state machine, which cancels a pending hop the moment a new recording starts so auto-hide can never get stuck) lives in `src/whisperwoof/ui/indicator/mando-sprite.ts` (+29 tests, including a drift guard that keeps the website demo constants in sync with the sprite manifest). The website's live demo plays the same sprites through the listen → polish → paste cycle, and the README's three steps show the animations as WebP loops (`website/mando/`).
